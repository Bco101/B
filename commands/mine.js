...
exports.run = async (client, message, args) => {
    ...
    let author = await db.fetch(`infoUser.${user.id}.cooldown.mine`)
	...
	if (author !== null && timeout - (Date.now() - author) > 0) {
        ...
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result]}`, amount)
        await db.set(`infoUser.${user.id}.cooldown.mine`, Date.now())
        ...
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result]}`, amount)
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result2]}`, amount2)
        await db.set(`infoUser.${user.id}.cooldown.mine`, Date.now())
        ...
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result]}`, amount)
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result2]}`, amount2)
        await db.add(`infoUser.${user.id}.inv.mine.${replies[result3]}`, amount3)
        await db.set(`infoUser.${user.id}.cooldown.mine`, Date.now())
