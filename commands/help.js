...
exports.run = async (client, message, args) => {
    ...
    .setColor("#ffb73b")
    .addField("คำสั่งทั่วไป", "`.avatar` `.dm` `.help` `.info` `.nick` `.say` `.getverify` `.verify` `.introduce` `.introduce-info`")
    ...
    .addField("คำสั่งแอดมิน", "`.ban` `.kick` `.purge` `.verify-admin` `.verify-role` `.question-add` `.question-admin` `.question-log` `.question-reset` `.question-role`");
